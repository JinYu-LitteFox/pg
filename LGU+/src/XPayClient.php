...
class XPayClient
{
    ...
    public $response_msg;
    public $log_file;
    public $err_label = ["FATAL", "ERROR", "WARN ", "INFO ", "DEBUG"];
    private $INFO
        = [
            "LGD_TXID",
            "LGD_AUTHCODE",
    ...
    function IsAcceptLog($ParamName, $LogLevel)
    {
        if (LGD_LOG_DEBUG == $LogLevel) {
            if (in_array($ParamName, $this->DEBUG, true)) {
                return true;
            }
        } elseif (LGD_LOG_INFO == $LogLevel) {
            if (in_array($ParamName, $this->INFO, true)) {
                return true;
            }
        }
    ...
    function __construct($home_dir, $mode = "real")
    {
        ...
        if (! file_exists($home_dir)) {
            ...
            $this->response_msg  = "home_dir [" . $home_dir
                                   . "] does not exist";
            trigger_error($this->response_msg, E_USER_ERROR);
        } elseif (! file_exists($home_dir . "/conf/mall.conf")) {
            $this->response_code = LGD_ERR_NO_MALL_CONFIG;
            $this->response_msg  = "config file [" . $home_dir
                                   . "/conf/mall.conf] does not exist";
            trigger_error($this->response_msg, E_USER_ERROR);
        } elseif (! file_exists($home_dir . "/conf/lgdacom.conf")) {
            $this->response_code = LGD_ERR_NO_LGDACOM_CONFIG;
            $this->response_msg  = "config file [" . $home_dir
                                   . "/conf/lgdacom.conf] does not exist";
            trigger_error($this->response_msg, E_USER_ERROR);
        }
        $array1         = parse_ini_file($home_dir . "/conf/mall.conf");
        $array2         = parse_ini_file($home_dir . "/conf/lgdacom.conf");
        $this->config   = $array1 + $array2;
        $this->log_file = $this->config["log_dir"] . "/log_" . date("Ymd")
                          . ".log";
    ...
    function init()
    {
        ...
        } elseif ($this->config['default_secure_protocols'] == "2048") {
            curl_setopt($this->ch, CURLOPT_SSLVERSION, 6);
            //echo "default_secure_protocols = " . $this->config['default_secure_protocols'] . "<br>";
        } elseif ($this->config['default_secure_protocols'] == "2560") {
    ...
    function Init_TX($MID)
    {
        if ($this->config[$MID] == null) {
            ...
            $this->response_msg  = "Key for MID [" . $MID
                                   . "] does not exist in mall.conf";
    ...
    function GenerateGUID()
    {
        if (function_exists('com_create_guid')) {
            ...
            mt_srand((double)microtime()
                     * 10000);//optional for php 4.2.0 and up.
    ...
    function set_curl_error()
    {
        ...
        $this->response_msg .= "; cURL error code = " . curl_errno($this->ch)
    ...
    function TX($bRollbackOnError = true)
    {
        ...
        if ($bCheckURL == true) {
            ...
            $this->log("TX failed: res code = " . $this->response_code
                       . "; msg = " . $this->response_msg, LGD_LOG_FATAL);
        }
        if ($result == false) {
            if ($bCheckURL == true) {
                $bTX = false;
                ...
                $this->log("TX failed: res code = " . $this->response_code
                           . "; msg = " . $this->response_msg, LGD_LOG_FATAL);
                if ($bRollbackOnError
                    && $this->Check_Auto_Rollback(curl_errno($this->ch))) {
                    $bRollback         = true;
            ...
            if ($http_res_code < 200 || $http_res_code >= 300) {
                ...
                if (($this->response_array == false)
                    || (strlen($this->response_array["LGD_RESPCODE"]) == 0)) {
                    // JSON decode failed
                    $bTX             = false;
                    $bReporting      = true;
                    ...
                    $this->response_code
                        = $this->response_array["LGD_RESPCODE"];
    ...
    function Report_TX()
    {
        ...
        if ($result == false) {
            ...
            $this->log("Reporting failed: res code = " . $this->response_code
                       . "; msg = " . $this->response_msg, LGD_LOG_ERROR);
        ...
        if ($http_res_code < 200 || $http_res_code >= 300) {
            // http response error
            $this->log("Reporting failed: HTTP response code = "
                       . $http_res_code, LGD_LOG_ERROR);
    ...
    function send_multipart_post_data(
        $url,
        $postdata,
        $file_field_array = [],
        $ip = null,
    ...
    {
        $LGD_HASHDATA = "";
        $LGD_HASHDATA = md5($LGD_MID . $LGD_OID . $LGD_AMOUNT . $LGD_TIMESTAMP
                            . $this->config[$LGD_MID]);
    ...
    function GetHashDataOpenpay($LGD_VENDERNO, $LGD_MID, $LGD_OPENPAY_MER_UID, $LGD_OPENPAY_TOKEN, $LGD_TIMESTAMP)
    {
        ...
        if (empty($LGD_OPENPAY_TOKEN)) {
            $InputData = $InputData . $LGD_TIMESTAMP . $this->config[$LGD_MID];
        } else {
            $InputData = $InputData . $LGD_OPENPAY_TOKEN . $LGD_TIMESTAMP
                         . $this->config[$LGD_MID];
        }
    ...
    function GetHashDataCas(
        ...
        $LGD_HASHDATA = "";
        $LGD_HASHDATA = md5($LGD_MID . $LGD_OID . $LGD_AMOUNT . $LGD_RESPCODE . $LGD_TIMESTAMP
                            . $this->config[$LGD_MID]);
    ...
    function DecodeAndDecrypt($MertKey, $EncryptAndEncodeBuffer)
    {
        ...
        return iconv("euc-kr", "utf-8", $UnPaddString);
    }
